...
SCRIPTS;
    ...
    private function varIsGDResource($imgRes): void
    {
        ...
        $filename = './imgs/tmp/image-' . uniqid('', true) . '.jpg';
    ...
    private function getVariableName()
    {
        ...
        for ($i = count($arrBacktrace) - 1; $i >= 0; $i--) {
            ...
            if (array_key_exists('function', $arrCurrent) && (in_array($arrCurrent['function'], $arrInclude, true) || (0 !== strcasecmp($arrCurrent['function'], 'dbug2')))) {
                continue;
            }
            $arrFile = $arrCurrent;
            break;
